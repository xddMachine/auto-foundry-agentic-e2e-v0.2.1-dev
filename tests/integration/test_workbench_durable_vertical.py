"""Complete offline proof of the v0.2.2 workbench/durable item path."""
...
def test_complete_offline_workbench_and_durable_vertical_path(tmp_path: Path) -> None:
    ...
    catalog = workbench.catalog(sample_rows=2, categorical_limit=3)
    ...
    assert workbench.data_room.read_rows(order_entry, limit=2) == [
    ...
    assert item.state["lifecycle_state"] == "recovery_ready"
    ...
    recovery = item.begin_recovery("lane-recovery", "Lead Analyst", prior_attempt_id=attempt.attempt_id)
    ...
    review = item.record_review("accept_with_limits", reviewer_ref="reviewer-2")
    ...
    assert (item.accepted_root / "final_answer.json").read_bytes() == reviewed_draft
    ...
    assert manifest["draft_hash"] == reviewed_draft_hash
...
def test_run_context_defaults_to_core_v020(tmp_path: Path) -> None:
    context = RunContext("RUN-DEFAULT-VERSION", tmp_path / "run")
    assert context.core_version == "0.2.0"
...
def test_item_state_template_loads_through_durable_core(tmp_path: Path) -> None:
    ...
    assert tuple(template) == base_fields
    assert tuple(ITEM_STATE_FIELDS[: len(base_fields)]) == base_fields
    assert set(template) <= set(ITEM_STATE_SCHEMA["fields"])

    context = RunContext("RUN-TEMPLATE", tmp_path / "run", core_version="0.2.0")
