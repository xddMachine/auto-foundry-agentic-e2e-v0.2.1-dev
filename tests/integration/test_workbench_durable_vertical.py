...
def test_run_context_defaults_to_current_versions(tmp_path: Path) -> None:
    ...
    assert context.core_version == "0.3.2"
    assert context.skill_version == "0.2.5"
...
def test_item_state_template_loads_through_durable_core(tmp_path: Path) -> None:
    ...
    context = RunContext("RUN-TEMPLATE", tmp_path / "run", core_version="0.3.2", skill_version="0.2.5")
