...
// The backend publishes hash-validated dashboard links under separate final
// and preview projections.  Keep the UI bound to those exact same-origin
// routes: a legacy/top-level run.dashboardUrl (or an invalid/tampered
// product sidecar) must never enable navigation.
function productRouteUrl(value, prefix) {
  if (typeof value !== "string" || !value || value.length > 512) return null;
  if ([...value].some((character) => character.charCodeAt(0) < 32)) return null;
  if (!value.startsWith(prefix) || !value.endsWith("/index.html")) return null;
  return value;
}

function reviewedDashboardUrl(snapshot) {
  const product = snapshot?.productDashboard;
  if (product?.valid !== true) return null;
  if (product?.source && product.source !== "accepted_candidate") return null;
  return productRouteUrl(product.dashboardUrl, "/api/product/dashboard/");
}

function previewDashboardUrl(snapshot) {
  const product = snapshot?.productPreview;
  if (product?.valid !== true) return null;
  if (product?.source && !["candidate", "incremental_preview"].includes(product.source)) return null;
  return productRouteUrl(product.previewUrl || product.dashboardUrl, "/api/product/preview/");
}

function dashboardLinkTarget(snapshot) {
  const finalUrl = reviewedDashboardUrl(snapshot);
  if (finalUrl) return { url: finalUrl, label: "Open dashboard", kind: "final" };
  const previewUrl = previewDashboardUrl(snapshot);
  if (previewUrl) return { url: previewUrl, label: "Open live preview", kind: "preview" };
  return null;
}

function renderDashboardLink(snapshot, dashboard) {
  if (!dashboard) return null;
  const target = dashboardLinkTarget(snapshot);
  if (target) {
    dashboard.href = target.url;
    dashboard.textContent = `${target.label} ↗`;
...
async function bootstrap() {
  ...
  try {
    ...
    // Apply the requested hash before waiting on the selected run's snapshot.
    // Large durable projections can take several seconds; navigation should
    // still show the requested view while that snapshot is loading.
    route();
...
function renderRunContext() {
  ...
  updateFreshness();
  renderDashboardLink(state.snapshot, $("#openDashboard"));
}
