#!/usr/bin/env python3
"""Serve the Auto Foundry Control Center without mutating runtime state.

Layer 1 is intentionally read-only.  It can render a deterministic fixture and
discover existing runs beneath explicitly configured roots.  The only POST
endpoint validates a launch draft; it never creates or changes a run.
"""
...
def _summarize_run_state(path: Path, payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    updated = _safe_text(
        _coalesce(payload, "updated_at", "last_updated_at", "created_at", default=""), 80
    )
    return {
...
class ReadOnlyRepository:
    ...
    def _filesystem_records(self) -> list[RunRecord]:
        ...
        records.sort(key=lambda record: str(record.summary.get("updatedAt", "")), reverse=True)
...
def _capacity_for_total(total: int) -> dict[str, int]:
    return {
        "total": total,
        "entityResolution": min(4, total),
        "analyticalOwner": min(1, total),
        "specialist": min(3, total),
    }


def _capacity_contract(value: Any) -> dict[str, int] | None:
    expected = {"total", "entityResolution", "analyticalOwner", "specialist"}
    if not isinstance(value, Mapping) or set(value) != expected:
        return None
    if any(isinstance(value[key], bool) or not isinstance(value[key], int) for key in expected):
        return None
    return {key: int(value[key]) for key in expected}


def _authoritative_capacity(run: Mapping[str, Any] | None) -> dict[str, int] | None:
    if not isinstance(run, Mapping):
        return None
    raw = run.get("capacity")
    if not isinstance(raw, Mapping):
        return None
    candidate = {
        "total": raw.get("total"),
        "entityResolution": raw.get("entityResolution"),
        "analyticalOwner": raw.get("analyticalOwner"),
        "specialist": raw.get("specialist"),
    }
    return _capacity_contract(candidate)


def validate_launch_draft(
    payload: Mapping[str, Any], known_runs: Mapping[str, Mapping[str, Any]] | set[str]
) -> dict[str, Any]:
    errors: dict[str, str] = {}
    mode = _safe_text(payload.get("mode"), 20)
    intake_blocks = payload.get("intakeBlocks")
    max_agents = payload.get("maxAgents")
    project_name = _safe_text(payload.get("projectName"), 120)
    run_id = _safe_text(payload.get("runId"), 80)
    source_url = _safe_text(payload.get("sourceUrl"), 500)
    sources = payload.get("sources")
    requested_capacity = _capacity_contract(payload.get("capacity"))
    known_run_map = known_runs if isinstance(known_runs, Mapping) else {run_id: {} for run_id in known_runs}

    if mode not in {"new", "continue"}:
        errors["mode"] = "Choose a new or existing run."
    if mode == "new" and len(project_name) < 3:
        errors["projectName"] = "Use at least 3 characters for the project/run name."
    if mode == "continue" and run_id not in known_run_map:
        errors["runId"] = "Select a discoverable run."
    has_text_intake = isinstance(intake_blocks, list) and any(_safe_text(item) for item in intake_blocks)
    has_document_intake = isinstance(sources, list) and bool(sources)
    if not has_text_intake and not has_document_intake:
        errors["intakeBlocks"] = "Enter a business brief, questions, or requirements."
    if not isinstance(max_agents, int) or not 1 <= max_agents <= 8:
        errors["maxAgents"] = "Agent capacity must be between 1 and 8."
    if requested_capacity is None:
        requested_capacity = _capacity_for_total(max_agents) if isinstance(max_agents, int) and 1 <= max_agents <= 8 else None
    if requested_capacity is not None:
        total = requested_capacity["total"]
        if not 1 <= total <= 8 or any(
            requested_capacity[key] < 0 or requested_capacity[key] > total
            for key in ("entityResolution", "analyticalOwner", "specialist")
        ):
            errors["maxAgents"] = "Role limits must be non-negative and cannot exceed total capacity."
        elif isinstance(max_agents, int) and max_agents != total:
            errors["maxAgents"] = "Total capacity does not match the max-agent control."
    if mode == "continue" and run_id in known_run_map:
        authoritative = _authoritative_capacity(known_run_map[run_id])
        if authoritative is None:
            errors["maxAgents"] = "Existing-run capacity is unavailable; this draft cannot promise a worker limit."
            requested_capacity = None
        else:
            if requested_capacity != authoritative:
                errors["maxAgents"] = "Existing-run capacity is authoritative and cannot be changed in this draft."
            requested_capacity = authoritative
    if source_url:
        errors["sourceUrl"] = "Remote URL ingestion is deferred; attach a local file or path."
    if sources is not None and not isinstance(sources, list):
        errors["sources"] = "Sources must be a list of local file descriptors."
    ...
    return {
        ...
        "effectiveCapacity": requested_capacity,
        "message": (
            "Draft is valid. Layer 1 deliberately does not start or change a run."
            if not errors
            else "Draft needs attention before a future launch command can be enabled."
        ),
        "checkedAt": _utc_now(),
    }
...
class ControlCenterHandler(BaseHTTPRequestHandler):
    server_version = "AutoFoundryControlCenter/0.1"

    @property
    def repository(self) -> ReadOnlyRepository:
        return self.server.repository  # type: ignore[attr-defined]

    def log_message(self, format: str, *args: Any) -> None:
        sys.stderr.write(f"[control-center] {self.address_string()} {format % args}\n")
    ...
    def _serve_static(self, request_path: str) -> None:
        ...
        if not _is_within(requested, (STATIC_DIR,)) or not requested.is_file():
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if parsed.path == "/api/health":
            self._json({"status": "ok", "mode": "read_only", "time": _utc_now()})
            return
        if parsed.path == "/api/config":
            self._json(
                {
                    "mode": "read_only_layer_1",
                    "maxAgents": 8,
                    "capacity": {"entityResolution": 4, "analyticalOwner": 1, "specialist": 3},
        ...
        if parsed.path == "/api/snapshot":
            ...
            try:
                ...
                self._error(HTTPStatus.UNPROCESSABLE_ENTITY, f"Could not project run: {_safe_text(error)}")
            return
        if parsed.path == "/api/events":
            ...
            try:
                ...
                self._error(HTTPStatus.UNPROCESSABLE_ENTITY, f"Could not read events: {_safe_text(error)}")
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        parsed = urlparse(self.path)
        if parsed.path != "/api/launch/validate":
            self._error(HTTPStatus.METHOD_NOT_ALLOWED, "No mutating commands are enabled")
            return
        try:
        ...
        if not isinstance(payload, dict):
            self._error(HTTPStatus.BAD_REQUEST, "Expected a JSON object")
            return
        known_runs = {run["id"]: run for run in self.repository.list_runs()}
        self._json(validate_launch_draft(payload, known_runs))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.host not in {"127.0.0.1", "localhost", "::1"}:
        raise SystemExit("Control Center Layer 1 binds to loopback only")
    ...
    server = ControlCenterServer((args.host, args.port), repository)
    print(f"Auto Foundry Control Center: http://{args.host}:{args.port}")
    print("Mode: read-only Layer 1 (launch endpoint validates drafts only)")
    try:
