...
class LaunchDraftValidationTests(unittest.TestCase):
    def test_valid_draft_is_explicitly_non_mutating(self) -> None:
        result = validate_launch_draft(
            {
                "mode": "new",
                "projectName": "Margin mission",
                "intakeBlocks": ["Explain margin erosion"],
                "sources": [{"name": "input.csv", "size": 12}],
                "sourceUrl": "",
                "maxAgents": 8,
        ...
        self.assertEqual(result["action"], "validate_only")

    def test_remote_url_and_invalid_capacity_are_rejected(self) -> None:
        result = validate_launch_draft(
            {
                "mode": "new",
                "projectName": "AB",
        ...
        self.assertEqual(set(result["errors"]), {"projectName", "intakeBlocks", "maxAgents", "sourceUrl"})

    def test_continue_requires_discoverable_run(self) -> None:
        result = validate_launch_draft(
            {
                "mode": "continue",
                "runId": "missing",
                "intakeBlocks": ["Add another question"],
                "sources": [],
                "sourceUrl": "",
                "maxAgents": 4,
            },
        ...
        self.assertEqual(result["errors"]["runId"], "Select a discoverable run.")

    def test_continue_uses_authoritative_existing_capacity(self) -> None:
        known = {
            "known": {
                "capacity": {
                    "total": 4,
        ...
        result = validate_launch_draft(
            {
                ...
                "sourceUrl": "",
                "maxAgents": 4,
                "capacity": {"total": 4, "entityResolution": 4, "analyticalOwner": 1, "specialist": 3},
            },
            known_runs=known,
        )
        self.assertTrue(result["valid"])
        self.assertEqual(result["effectiveCapacity"]["total"], 4)

    def test_role_capacity_cannot_exceed_total(self) -> None:
        result = validate_launch_draft(
            {
                "mode": "new",
                "projectName": "Small mission",
                "intakeBlocks": ["Check one thing"],
                "sources": [],
                "sourceUrl": "",
                "maxAgents": 2,
                "capacity": {"total": 2, "entityResolution": 4, "analyticalOwner": 1, "specialist": 2},
            },
            known_runs=set(),
        )
        self.assertFalse(result["valid"])
        self.assertIn("cannot exceed", result["errors"]["maxAgents"])
