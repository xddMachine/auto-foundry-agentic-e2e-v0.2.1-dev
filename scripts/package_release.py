...
def _build_wheel(root: Path, dist: Path) -> Path:
    before = set(dist.glob("auto_foundry_core-0.1.0-*.whl"))
    command = [
        sys.executable,
        "-m",
        "pip",
    ...
    after = sorted(set(dist.glob("auto_foundry_core-0.1.0-*.whl")) - before)
    if len(after) != 1:
...
def package_release(root: Path, dist: Path) -> dict[str, object]:
    ...
    if zip_path.exists():
        zip_path.unlink()
    for wheel in dist.glob("auto_foundry_core-0.1.0-*.whl"):
        wheel.unlink()
