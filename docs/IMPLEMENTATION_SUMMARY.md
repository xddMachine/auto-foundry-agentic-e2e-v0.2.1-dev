...
- `auto_foundry_core` v0.3.5 is a small, source-agnostic deterministic
...
- `auto-foundry-agentic-e2e` v0.2.8 is a natural reviewed workflow with
...
- `analysis.py`, `integration.py`, `lifecycle.py`, `prepared.py`, and
  `product_contracts.py` provide the current v0.3.5 public runtime,
...
`tests/integration/test_workbench_durable_vertical.py` proves the normal v0.2.8
...
`ItemWorkspace.discard_business_review(...)` is reserved for an inadmissible,
item-bound `reviewer_scope` packet. It writes an append-only hash-bound audit,
atomically removes the packet while preserving work/draft bytes, resets review
and repair state, and requires a new full review without semantic
reinterpretation or a compatibility fallback.

`BoundAnalysisContext.rebind_implementation(...)` is the public resumable
identity-transition route. Its contiguous ledger binds old/new skill/core
versions and SHA/tree identities, earliest affected item, preserved accepted
hashes, revalidation reason, and resume point. It reuses the same source,
catalog, stat signature, and physical inventory; durable intent, append-only
audit, and anchored heads recover process loss under journal → run → item lock
order without ZIP/member reads, catalog rebuilds, counter changes, false
telemetry, new analysis, or raw reads. Active attempt/review/terminal/accepted
state is rejected, and invalid reviewer packets must be discarded first.

For later or multi-hop items, `BoundAnalysisContext.create_from_transitioned_catalog(...)`
creates immutable source inheritance rather than a synthetic transition. It
reuses the original source/catalog/stat/inventory identity with no ZIP/member
discovery, catalog rebuild, inventory-counter change, or raw read. Recursive
upstream provenance is validated under inherited journals oldest first → target
inheritance journal → run → lexical source/target item locks. Target
intent/manifest/record/state phases recover idempotently after a crash and no
synthetic target transition audit is written. `earliest_affected_item` is a
lower bound that covers later items only.
...
**v0.2.8 / core 0.3.5 — offline program validation complete for later Benchmark A**. Benchmark A
